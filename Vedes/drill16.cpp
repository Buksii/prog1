#include "Header/GUI.h"
 //clang++ -std=c++11 drill16.cpp Header/Graph.cpp Header/Window.cpp Header/GUI.cpp Header/Simple_window.cpp -o drill16 `fltk-config --ldflags --use-images`
#include <iostream>
...
Lines_window::Lines_window(Point xy, int w, int h, const string& title) //belső tartalom, öröklődés a guitól
        : Window{xy, w, h, title},
          next_button{Point{x_max() - 150, 0}, 70, 20, "Next point",
                      [](Address, Address pw) { reference_to<Lines_window>(pw).next(); }},
          quit_button{Point{x_max() - 70, 0}, 70, 20, "Quit",
                      [](Address, Address pw) { reference_to<Lines_window>(pw).quit(); }},
          next_x{Point{x_max() - 310, 0}, 50, 20, "next x:"}, //inbox_widgetből öröklődik {pozi x, y} [doboz méret] "név"
...
int main()
try {
    Lines_window win {Point{100, 100}, 600, 400, "lines"}; //értékadás az ablaknak
    return gui_main();
}
catch(exception& e) {
    cerr << "Error: " << e.what() << '\n';
